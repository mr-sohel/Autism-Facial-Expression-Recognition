...
- Run the full pipeline (all 15 models sequentially): `python run_experiments.py`
...
This repository is an evaluation framework comparing 15 deep learning architectures (CNNs, Vision Transformers, and a custom hybrid model) for Facial Expression Recognition in individuals with Autism Spectrum Disorder (ASD). 

**Data & Imbalance Handling**
- The dataset (`master_dataset_split/`) suffers from severe class imbalance. This is primarily handled in `src/dataset.py` using a custom `WeightedRandomSampler`, and in training via class-weighted losses (or Focal Loss).

**Code Modularity (`src/`)**
- `src/models.py`: Centralized model definition. It contains `MODEL_CONFIGS` (a dictionary mapping model names to their architecture configs), the factory function `get_model()`, and the definition for the custom `EssWin6` dual-branch hybrid model (EfficientNet-V2-S + Swin-B).
